        ...
        public override void OnHitNPC(Player player, NPC target, NPC.HitInfo hit, int damageDone)
        {
            if (UniverseUtils.IsAValidTarget(target))
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    ...
                    Projectile.NewProjectileDirect(target.GetSource_OnHit(target), player.Center + newVel * 8f, newVel, player.beeType(), player.beeDamage(damageDone), player.beeKB(0f), player.whoAmI);
                }
            }
        }
