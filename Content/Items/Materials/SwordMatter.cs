...
namespace UniverseOfSwordsMod.Content.Items.Materials
{
    public class SwordMatter : ModItem
    {
        ...
        public override bool PreDrawInWorld(SpriteBatch spriteBatch, Color lightColor, Color alphaColor, ref float rotation, ref float scale, int whoAmI)
        {
            ...
            Vector2 drawPosGlow = Item.position - new Vector2(Item.width / 2, Item.height / 2) - Main.screenPosition;
            float timer = (float)Main.timeForVisualEffects / 120f;
            for (float i = 0f; i < 1f; i += 0.25f)
            {
                spriteBatch.Draw(texture, drawPos + new Vector2(0f, 5f).RotatedBy(MathHelper.Pi * timer * i), texture.Frame(), new Color(255, 255, 255, 0) * 0.5f, rotation, drawOrigin, scale, SpriteEffects.None, 0f);
            }
