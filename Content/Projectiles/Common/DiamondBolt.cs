...
namespace UniverseOfSwordsMod.Content.Projectiles.Common
{
    public class DiamondBolt : ModProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int j = 0; j < 15; j++)
            {
                Dust dust = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, DustID.GemDiamond, Projectile.oldVelocity.X, Projectile.oldVelocity.Y, 50, default, 1.25f);
                dust.noGravity = true;
                Dust dust2 = dust;
                dust2.scale *= 1.25f;
                dust2 = dust;
