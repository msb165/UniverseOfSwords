...
namespace UniverseOfSwords.Content.Dusts
{
    public class BlackOre : ModDust
    {
        ...
        {
            dust.velocity *= 0.2f; // Mulitiply the Velocity on both X and Y by a float (Example 0.6f)
            dust.noGravity = true; // Dust doesn't fall to the ground
            dust.noLight = true; // Dust doesn't emit light
            dust.scale *= 3f; // The scale of the graphic (Default: 1f)
        }

        public override bool Update(Dust dust)
        {
            dust.position += dust.velocity; // Moves the dust according to the velocity.
            dust.rotation += dust.velocity.X * 0.5f; // Will rotate the dust based on Velocity.
